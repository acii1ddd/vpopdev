...
if [ ! -s "$PGDATA/PG_VERSION" ]; then
    ...
    log "Waiting for master to accept connections..."
    until pg_isready -h master -p 5432 -U replicator -d devpops; do
        log "Master not ready yet..."
        sleep 2
    done
    log "Master is ready. Starting backup..."
